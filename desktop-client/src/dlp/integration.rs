...
impl DlpIntegration {
    ...
    pub async fn sanitize_for_storage(&self, content: &str) -> DlpResult<String> {
        ...
        if result.was_blocked {
            ...
            return Err(DlpError::Sanitization(result.block_reason.unwrap_or_else(
                || "Content contains critical sensitive data".to_string(),
            )));
        }
    ...
    async fn build_patterns(config: &DlpIntegrationConfig) -> DlpResult<Vec<LeakPattern>> {
        ...
        for custom_config in &config.custom_patterns {
            ...
            match custom_pattern.to_leak_pattern() {
                Ok(pattern) => {
                    debug!(pattern_name = pattern.name, "Added custom DLP pattern");
        ...
        for cp in custom_patterns {
            ...
            if let Some(ref replacement) = cp.replacement {
                if !replacement.is_empty() {
                    sanitization
                        .replacement_map
                        .insert(cp.name.clone(), replacement.clone());
                }
            }
        }
    }
...
mod tests {
    ...
    async fn test_check_http_request_with_secret() {
        ...
        let result = integration
            .check_http_request("https://evil.com/steal?key=LTAI4G8aB9cD2eFgH3iJ", &[], None)
            .await;
