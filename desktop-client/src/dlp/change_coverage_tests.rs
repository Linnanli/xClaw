...
mod change_coverage_tests {
    ...
    use crate::dlp::{DlpDetector, DlpIntegration, DlpSanitizer, SanitizationConfig};
    ...
    async fn test_change_refactoring_behavior_consistency() {
        ...
        for (i, (desc, content)) in test_scenarios.iter().enumerate() {
            let new_result = integration_after_refactor
                .scan_user_input(content)
                .await
                .unwrap();
            ...
            // 关键行为应该保持一致
            assert_eq!(
                new_result.had_sensitive_data, baseline_result.had_sensitive_data,
                "Sensitive data detection changed for: {}",
                desc
            );
            assert_eq!(
                new_result.was_blocked, baseline_result.was_blocked,
                "Blocking behavior changed for: {}",
                desc
            );
            ...
            if baseline_result.had_sensitive_data && !baseline_result.was_blocked {
                assert!(
                    new_result.sanitized_content.contains("*")
                        == baseline_result.sanitized_content.contains("*"),
                    "Sanitization behavior changed for: {}",
                    desc
                );
            }
        }
    ...
    fn test_change_detector_algorithm_consistency() {
        ...
        for input in &test_inputs {
            ...
            // 检测结果应该一致
            assert_eq!(
                result_v1.has_sensitive_data, result_v2.has_sensitive_data,
                "Detection inconsistency for: {}",
                input
            );
            assert_eq!(
                result_v1.should_block, result_v2.should_block,
                "Blocking inconsistency for: {}",
                input
            );
            assert_eq!(
                result_v1.matches.len(),
                result_v2.matches.len(),
                "Match count inconsistency for: {}",
                input
            );
        }
    }
    ...
    async fn test_change_performance_regression_detection() {
        ...
        let performance_test_cases = [
            ...
            (
                "中等文本",
                &("测试内容 ".repeat(100) + "身份证： 110101199003071234 "),
            ),
            (
                "大文本",
                &("测试内容 ".repeat(1000) + "身份证： 110101199003071234 "),
            ),
        ];
...
mod regression_tests {
    ...
    async fn test_regression_issue_001_chinese_id_detection() {
        ...
        for case in &problematic_cases {
            ...
            assert!(
                ...
                "Regression: ID card not detected in '{}'",
                case
            );
        }
    }
    ...
    async fn test_regression_issue_003_api_key_blocking() {
        ...
        for api_key in &api_key_cases {
            ...
            assert!(
                result.had_sensitive_data,
                "Regression: API key not detected: {}",
                api_key
            );
            ...
            if api_key.starts_with("LTAI") || api_key.starts_with("sk-proj") {
                assert!(
                    result.was_blocked,
                    "Regression: Critical API key not blocked: {}",
                    api_key
                );
            }
        }
    ...
    async fn test_regression_issue_004_concurrent_safety() {
        // 回归测试：并发安全问题
        let integration = std::sync::Arc::new(DlpIntegration::with_default_config().await.unwrap());
        ...
        for (i, handle) in handles.into_iter().enumerate() {
            ...
            assert!(
                result.is_ok(),
                "Regression: Concurrent request {} failed",
                i
            );
            ...
            assert!(
                ...
                "Regression: Concurrent detection failed for request {}",
                i
            );
        }
    }
    ...
    fn test_regression_issue_005_pattern_compilation() {
        ...
        assert!(patterns.len() > 0, "Regression: No patterns loaded");
