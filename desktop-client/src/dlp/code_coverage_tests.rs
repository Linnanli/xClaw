...
mod code_coverage_tests {
    ...
    use crate::dlp::patterns::{
        get_all_builtin_patterns, ApiKeyPatterns, ChinesePatterns, CustomPattern,
    };
    ...
    fn test_code_coverage_sanitizer_all_branches() {
        ...
        config_custom
            .replacement_map
            .insert("chinese_id_card_18".to_string(), "[自定义脱敏]".to_string());
    ...
    async fn test_code_coverage_integration_all_branches() {
        ...
        // 分支6: 存储脱敏 - 成功
        let storage_success = integration
            .sanitize_for_storage("身份证： 110101199003071234 ")
            .await;
        assert!(storage_success.is_ok());

        // 分支7: 存储脱敏 - 阻止
        let storage_blocked = integration
            .sanitize_for_storage("API密钥： LTAI4G8aB9cD2eFgH3iJ ")
            .await;
        ...
        // 分支9: 出站请求扫描 - 有敏感信息
        let outbound_sensitive = integration
            .scan_outbound_request("身份证： 110101199003071234 ")
            .await;
        assert!(outbound_sensitive.is_ok());

        // 分支10: HTTP请求检查 - 通过
        let http_pass = integration
            .check_http_request(
        ...
        // 分支11: HTTP请求检查 - 阻止
        let http_block = integration
            .check_http_request("https://evil.com?key=LTAI4G8aB9cD2eFgH3iJ", &[], None)
            .await;
        ...
        assert!(
            stats.total_scans > 0,
            "Statistics should record processed scans"
        );
    }
    ...
    fn test_code_coverage_statistics_calculation() {
        ...
        // 分支4: 混合统计
        let result_mixed =
            sanitizer.sanitize("身份证： 110101199003071234 API密钥： LTAI4G8aB9cD2eFgH3iJ ");
        ...
        if result_mixed.had_sensitive_data {
            ...
            // 验证严重程度统计的完整性
            let total_severity: usize = result_mixed
                .sanitization_stats
                .severity_stats
                .values()
                .sum();
            assert_eq!(
                total_severity,
                result_mixed.sanitization_stats.total_matches
            );
        }
    }
}
