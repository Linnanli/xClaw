...
mod data_coverage_tests {
    use crate::dlp::{DlpDetector, DlpSanitizer, SanitizationConfig, DlpIntegration, DlpSeverity, DlpAction};
    ...
    fn test_data_coverage_severity_enum_values() {
        ...
        for i in 1..severities.len() {
            assert!(severities[i] > severities[i-1], "Severity ordering should be consistent");
        }
    ...
    fn test_data_coverage_string_data_types() {
        ...
        for (desc, content) in &string_test_cases {
            ...
            assert!(
                ...
                "Should detect ID in {} string: {}", desc, content
            );
        }
    }
    ...
    fn test_data_coverage_invalid_data_handling() {
        ...
        for (desc, invalid_data) in &invalid_data_cases {
            ...
            println!("Tested invalid {}: {} -> detected: {}", 
    ...
    fn test_data_coverage_null_and_empty_values() {
        ...
        for (desc, empty_content) in &empty_test_cases {
            ...
            assert!(
                !result.has_sensitive_data,
                "Empty content should not be detected as sensitive: {}", desc
            );
        }
    }
    ...
    fn test_data_coverage_configuration_data_types() {
        ...
        for enabled in &bool_values {
            ...
            if *enabled {
                assert!(result.had_sensitive_data || !result.had_sensitive_data); // 任一结果都可接受
            } else {
    ...
    async fn test_data_coverage_custom_pattern_data_types() {
        ...
        for severity_str in &severity_strings {
            for action_str in &action_strings {
                ...
                assert!(integration.is_ok(), "Should create integration with {} {}", severity_str, action_str);
                
                let integration = integration.unwrap();
                let result = integration.scan_user_input("测试：1234-5678").await.unwrap();
    ...
    fn test_data_coverage_pattern_regex_data_types() {
        ...
        for (desc, pattern, test_input) in &regex_test_cases {
            ...
            assert!(leak_pattern_result.is_ok(), "Pattern should compile: {} ({})", desc, pattern);
    ...
    fn test_data_coverage_collection_data_types() {
        ...
        for content in &string_vec {
            let result = detector.scan(content);
            println!("Vec test - Content: '{}', Detected: {}", content, result.has_sensitive_data);
        }
        ...
        if result.had_sensitive_data && !result.was_blocked {
            assert!(result.sanitized_content.contains("[身份证]") || result.sanitized_content.contains("*"));
        }
        ...
        for opt_content in &optional_strings {
            match opt_content {
                Some(content) => {
                    let result = detector.scan(content);
                    println!("Option test - Content: '{}', Detected: {}", content, result.has_sensitive_data);
                }
                None => {
