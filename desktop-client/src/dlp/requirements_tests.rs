...
mod requirements_tests {
    ...
    async fn req_dlp_001_chinese_id_card_detection() {
        ...
        for (desc, input, should_detect) in &test_cases {
            ...
            assert_eq!(
                result.had_sensitive_data, *should_detect,
                "Failed for: {}",
                desc
            );
            ...
            if *should_detect {
                assert!(
                    !result.was_blocked,
                    "ID card should be redacted, not blocked"
                );
                assert!(
    ...
    async fn req_dlp_003_api_key_blocking() {
        ...
        let test_cases = [
            (
                "阿里云AccessKey",
                "密钥： LTAI4G8aB9cD2eFgH3iJ ",
                true,
                true,
            ),
            (
                "腾讯云SecretId",
                "密钥： AKIDabcdefghijklmnopqrstuvwxyz123456 ",
                true,
                true,
            ),
            (
                "OpenAI API Key",
                "密钥： sk-proj-abc123def456ghi789jkl012mno345pqrT3BlbkFJtest123 ",
                true,
                true,
            ),
            (
                "GitHub Token",
                "密钥： ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx ",
                true,
                true,
            ),
        ...
        for (desc, input, should_detect, should_block) in &test_cases {
            ...
            assert_eq!(
                result.was_blocked, *should_block,
                "Blocking failed for: {}",
                desc
            );
        }
    ...
    async fn req_dlp_004_http_request_protection() {
        ...
        let test_cases = [
            ...
            (
                "URL中的身份证",
                "https://api.com/user?id=110101199003071234",
                false,
            ), // 身份证是脱敏级别，不阻止
        ...
        let header_tests = [
            (
                ...
                vec![(
                    ...
                    "Bearer LTAI4G8aB9cD2eFgH3iJ".to_string(),
                )],
                true,
            ),
            (
        ...
        for (desc, headers, should_block) in &header_tests {
            let result = integration
                .check_http_request("https://api.example.com", &headers, None)
                .await;
            if *should_block {
    ...
    async fn req_dlp_008_custom_rules() {
        ...
        // 测试自定义规则
        let result = integration
            .scan_user_input("员工工号： EMP-123456 ")
            .await
            .unwrap();
        assert!(result.had_sensitive_data);
    ...
    async fn req_dlp_008c_custom_rules_default_replacement() {
        ...
        let result = integration
            .scan_user_input("代码：SECRET-1234")
            .await
            .unwrap();
        assert!(result.had_sensitive_data);
        assert!(
    ...
    async fn req_dlp_009_performance_requirements() {
        ...
        for input in &test_cases {
            ...
            assert!(
                ...
                "Performance requirement failed for input length: {}",
                input.len()
            );
        }
    }
...
mod business_scenario_tests {
    ...
    async fn scenario_user_registration() {
        ...
        // 1. 前端输入验证
        let input_result = integration
            .scan_user_input(registration_data)
            .await
            .unwrap();
        ...
        // 2. 存储到数据库前脱敏
        let storage_data = integration
            .sanitize_for_storage(registration_data)
            .await
            .unwrap();
        ...
        let outbound_result = integration
            .scan_outbound_request(&api_payload)
            .await
            .unwrap();
    ...
    async fn scenario_api_integration() {
        ...
        let api_requests = [
            ...
            (
                ...
                Some(r#"{"name":"张三","id":"110************234"}"#),
            ),
            (
                "POST",
                "https://evil.com/steal",
                Some(r#"{"secret":"LTAI4G8aB9cD2eFgH3iJ"}"#),
            ),
        ];
