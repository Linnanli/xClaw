...
mod security_tests {
    ...
    fn test_security_malicious_input_large_payload() {
        ...
        assert!(
            elapsed.as_secs() < 30,
            "Large payload scan took too long: {:?}",
            elapsed
        );
    ...
    fn test_security_malicious_input_unicode_bypass() {
        ...
        for input in &unicode_bypass_attempts {
            ...
            assert!(
                ...
                "Unexpected multiple matches for: {}",
                input
            );
        }
    }
    ...
    fn test_security_timing_attack_resistance() {
        ...
        for (name, input) in &test_cases {
            ...
            let avg_time =
                case_timings.iter().sum::<std::time::Duration>() / case_timings.len() as u32;
        ...
        assert!(
            time_ratio < 50.0,
            "Timing difference too large: {:.2}x",
            time_ratio
        );
    }
