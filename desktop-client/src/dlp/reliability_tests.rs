...
mod reliability_tests {
    use crate::dlp::{DlpIntegration, DlpDetector, DlpSanitizer};
    ...
    async fn test_reliability_configuration_corruption_recovery() {
        ...
        // 正常配置测试
        let result1 = integration.scan_user_input("身份证： 110101199003071234 ").await.unwrap();
        assert!(result1.had_sensitive_data);
        ...
        // 验证系统仍然可以工作（虽然功能被禁用）
        let result2 = integration.scan_user_input("身份证： 110101199003071234 ").await.unwrap();
        ...
        // 验证功能恢复
        let result3 = integration.scan_user_input("身份证： 110101199003071234 ").await.unwrap();
    ...
    async fn test_reliability_resource_exhaustion_protection() {
        ...
        for (i, input) in resource_intensive_inputs.iter().enumerate() {
            ...
            // 应该在合理时间内完成（<5秒）
            assert!(elapsed.as_secs() < 5, "Resource test {} took too long: {:?}", i, elapsed);
    ...
    async fn test_reliability_timeout_handling() {
        ...
        let timeout_test = async {
            let result = integration.scan_user_input("身份证： 110101199003071234 ").await;
            result
        };
...
mod stress_tests {
    ...
    async fn test_stress_high_throughput() {
        ...
        for worker_id in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                for i in 0..100 {
                    let content = format!("工作线程 {} 请求 {} 身份证： 11010119900307123{} ", 
                                        worker_id, i, i % 10);
    ...
    async fn test_stress_pattern_matching_performance() {
        ...
        println!("Average scan time: {:.2}ms, Total matches: {}", avg_time_per_scan, total_matches);
        
        // 平均扫描时间应该小于10ms
        assert!(avg_time_per_scan < 10.0, "Average scan time too high: {:.2}ms", avg_time_per_scan);
