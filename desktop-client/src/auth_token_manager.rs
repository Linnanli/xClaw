...
impl AuthTokenManager {
    ...
    pub fn load_or_generate(&self) -> Result<String, TokenError> {
        ...
        fs::write(&self.token_file, &token).map_err(|e| TokenError::IoError(e.to_string()))?;
    ...
    pub fn load(&self) -> Result<String, TokenError> {
        ...
        let token =
            fs::read_to_string(&self.token_file).map_err(|e| TokenError::IoError(e.to_string()))?;
    ...
    pub fn delete(&self) -> Result<(), TokenError> {
        if platform_utils::file_exists(&self.token_file) {
            fs::remove_file(&self.token_file).map_err(|e| TokenError::IoError(e.to_string()))?;
        }
        Ok(())
    }
...
impl std::fmt::Display for TokenError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TokenError::InvalidCharacter(details) => {
                write!(f, "Token contains invalid characters: {}", details)
            }
            TokenError::InvalidLength(len) => {
                write!(f, "Invalid token length: {} (expected 64)", len)
            }
...
mod tests {
    ...
    fn test_token_error_display() {
        ...
        assert_eq!(
            TokenError::InvalidPath.to_string(),
    ...
    fn test_token_cleaning_from_db_format() {
        ...
        assert_eq!(
            cleaned.len(),
            64,
            "Token should be 64 characters after cleaning"
        );
        assert!(
        ...
        assert!(
            !cleaned.contains('\r'),
            "Token should not contain carriage returns"
        );
    ...
    fn test_token_with_newlines_should_be_cleaned() {
        ...
        assert!(
            result.is_ok(),
            "Token with newline in middle should be cleaned successfully"
        );
        ...
        // 末尾的换行符也应该能够清理
        let db_value_with_trailing_newline =
    ...
    fn test_security_malicious_input_protection() {
        ...
        for (i, token) in malicious_tokens.iter().enumerate() {
            ...
            assert!(
                clean_token(token).is_err(),
                "Should fail to clean malicious token {}: {}",
                i,
                token
            );
        }
    ...
    fn test_security_side_channel_resistance() {
        ...
        let inputs = vec![
            "", "a", "ab", "abc", &input_32, &input_63, &input_64, &input_65, &input_128,
        ];
