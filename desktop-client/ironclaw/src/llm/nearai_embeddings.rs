...
//!
//! Uses the same session-based auth as the LLM provider. Lives in `crate::llm`
//! (not `dasclaw_workspace_cap`) because it depends on [`SessionManager`] and
//! [`crate::llm::retry::parse_retry_after`], both ironclaw business-layer
//! types.
