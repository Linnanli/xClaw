...
//!
//! The production implementation now lives in [`x_claw_agent::compaction`]
//! and works against the host traits [`x_claw_agent::WorkspaceWriter`] and
//! [`x_claw_agent::LlmCompleter`]. Ironclaw ships blanket impls for
...
//! integration tests that can't live in `x_claw_agent` (the crate must stay
//! free of the `libsql` backend).

pub use x_claw_agent::compaction::{CompactionResult, ContextCompactor};
...
mod libsql_tests {
    ...
    use x_claw_agent::compaction::ContextCompactor;
    use x_claw_agent::context_monitor::CompactionStrategy;
    use x_claw_agent::session::Thread;
