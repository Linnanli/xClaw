...
async fn req_auth_001_token_format() {
    ...
    assert!(
        ...
        "REQ-AUTH-001: Valid token should pass validation"
    );

    // 3. 验证函数必须拒绝无效Token
    assert!(
        !is_valid_token("invalid"),
        "REQ-AUTH-001: Invalid token should fail validation"
    );
    assert!(
        !is_valid_token(&"g".repeat(64)),
        "REQ-AUTH-001: Non-hex token should fail validation"
    );
    assert!(
        !is_valid_token(&"a".repeat(63)),
        "REQ-AUTH-001: Short token should fail validation"
    );
    assert!(
        !is_valid_token(&"a".repeat(65)),
        "REQ-AUTH-001: Long token should fail validation"
    );
...
async fn req_auth_002_auto_retry() {
    ...
    assert_ne!(
        ...
        "REQ-AUTH-002: Refreshed token should be different"
    );

    // 3. 验证新Token有效
    assert!(
        is_valid_token(&new_token),
        "REQ-AUTH-002: Refreshed token should be valid"
    );

    // 4. 验证能够保存和加载新Token
    assert!(
        token_manager.save(&new_token).is_ok(),
        "REQ-AUTH-002: Should be able to save refreshed token"
    );
...
async fn req_auth_003_token_persistence() {
    ...
    assert!(
        token_file.exists(),
        "REQ-AUTH-003: Token file should exist after save"
    );
    ...
    assert_eq!(
        token,
        loaded_token.trim(),
        "REQ-AUTH-003: Loaded token should match saved token"
    );
    ...
    assert_eq!(
        token,
        loaded_token2.trim(),
        "REQ-AUTH-003: Token should persist across sessions"
    );
...
async fn req_auth_004_token_source_priority() {
    ...
    assert!(
        tauri_token.is_ok(),
        "REQ-AUTH-004: Tauri command should be able to get token"
    );

    let token = tauri_token.unwrap();
    assert!(
        is_valid_token(&token),
        "REQ-AUTH-004: Token from Tauri command should be valid"
    );
    ...
    assert!(
        is_valid_token(&local_token),
        "REQ-AUTH-004: Local storage token should be valid"
    );
...
async fn req_auth_005_token_cleaning() {
    ...
    assert_eq!(
        cleaned.len(),
        64,
        "REQ-AUTH-005: Cleaned token should be 64 characters"
    );
    assert!(
        is_valid_token(&cleaned),
        "REQ-AUTH-005: Cleaned token should be valid"
    );
    ...
    assert!(
        is_valid_token(&cleaned),
        "REQ-AUTH-005: Whitespace-cleaned token should be valid"
    );
    ...
    assert!(
        is_valid_token(&cleaned),
        "REQ-AUTH-005: Newline-cleaned token should be valid"
    );

    // 4. 测试无效Token拒绝
    assert!(
        clean_token("invalid").is_err(),
        "REQ-AUTH-005: Invalid token should be rejected"
    );
    assert!(
        clean_token("").is_err(),
        "REQ-AUTH-005: Empty token should be rejected"
    );
...
async fn req_auth_006_concurrent_safety() {
    ...
    for handle in handles {
        ...
        assert!(
            result.is_ok(),
            "REQ-AUTH-006: Concurrent access {} should succeed",
            i
        );
    ...
    for (i, token) in results.iter().enumerate().skip(1) {
        assert_eq!(
            first_token, token,
            "REQ-AUTH-006: Concurrent token {} should be consistent",
            i
        );
    }
...
async fn req_auth_007_error_handling() {
    ...
    assert!(
        invalid_save.is_err(),
        "REQ-AUTH-007: Invalid token save should be rejected"
    );
    ...
    assert!(
        empty_save.is_err(),
        "REQ-AUTH-007: Empty token save should be rejected"
    );

    // 3. 测试从错误状态恢复
    let recovery_token = token_manager.load_or_generate();
    assert!(
        recovery_token.is_ok(),
        "REQ-AUTH-007: Should recover from error state"
    );

    let token = recovery_token.unwrap();
    assert!(
        is_valid_token(&token),
        "REQ-AUTH-007: Recovered token should be valid"
    );
...
async fn req_auth_008_security_requirements() {
    ...
    for input in malicious_inputs {
        assert!(
            !is_valid_token(input),
            "REQ-AUTH-008: Malicious input should be rejected: {}",
            input
        );
        assert!(
            clean_token(input).is_err(),
            "REQ-AUTH-008: Malicious input cleaning should fail: {}",
            input
        );
    }
    ...
    assert_ne!(
        token1, token2,
        "REQ-AUTH-008: Generated tokens should be random"
    );
    ...
    for i in 0..tokens.len() {
        for j in i + 1..tokens.len() {
            assert_ne!(
                tokens[i], tokens[j],
                "REQ-AUTH-008: Tokens should be unpredictable"
            );
        }
    }
...
async fn req_auth_009_performance_requirements() {
    ...
    assert!(
        ...
        "REQ-AUTH-009: Token generation should be fast"
    );
    ...
    assert!(
        ...
        "REQ-AUTH-009: Token validation should be fast"
    );
    ...
    assert!(
        cleaning_time.as_millis() < 100,
        "REQ-AUTH-009: Token cleaning should be fast"
    );

    println!("✅ REQ-AUTH-009: Performance requirements verified");
    println!(
        "   Generation: {}ms/100, Validation: {}ms/1000, Cleaning: {}ms/1000",
        generation_time.as_millis(),
        validation_time.as_millis(),
        cleaning_time.as_millis()
    );
}
...
async fn req_auth_010_compatibility_requirements() {
    ...
    for old_token in old_format_tokens {
        ...
        assert!(
            cleaned.is_ok(),
            "REQ-AUTH-010: Old format token should be compatible: {:?}",
            old_token
        );
        ...
        assert_eq!(
            token.len(),
            64,
            "REQ-AUTH-010: Cleaned old token should be 64 chars"
        );
        assert!(
            is_valid_token(&token),
            "REQ-AUTH-010: Cleaned old token should be valid"
        );
    }
    ...
    assert!(
        token_path.is_absolute() || token_path.starts_with("~"),
