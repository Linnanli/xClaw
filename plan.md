...
- 前置：SKILL.md 格式校验（安全扫描之前执行）：
  ...
  - 校验必填字段：name（字母数字开头，允许点/下划线/连字符，最长 64 字符）、version（非空）、description（非空）、activation（存在且 keywords/patterns 至少一个非空）
...
- 测试：extensions_unit_tests.rs 更新（格式校验失败路径 + 扫描流程路径）
