...
//! 1. Summarizes old turns via an [`LlmCompleter`]
//! 2. Writes the summary to a workspace daily log via a [`WorkspaceWriter`]
...
use crate::messages::{ChatMessage, CompletionRequest, Role};
use crate::session::{Thread, Turn};
use crate::traits::{HostError, LlmCompleter, WorkspaceWriter};
...
impl ContextCompactor {
    ...
    pub fn new(llm: Arc<dyn LlmCompleter>) -> Self {
        Self { llm }
    }
    ...
    pub async fn compact(
        ...
        workspace: Option<&dyn WorkspaceWriter>,
    ...
    async fn compact_with_summary(
        ...
        keep_recent: usize,
        workspace: Option<&dyn WorkspaceWriter>,
    ) -> Result<CompactionPartial, HostError> {
    ...
    async fn generate_summary(&self, messages: &[ChatMessage]) -> Result<String, HostError> {
...
Be brief but capture all important details. Use bullet points."#,
        ...
        self.llm.complete_text(request).await
    }
...
fn format_turns_for_storage(turns: &[Turn]) -> String {
    turns
        .iter()
        .map(|turn| {
...
mod tests {
    ...
    /// Minimal `LlmCompleter` stub for tests. Returns a canned text or a
    /// canned error, and counts calls. Mirrors the surface of ironclaw's
    /// `StubLlm` that the pre-port tests used.
    ...
    #[async_trait]
    impl LlmCompleter for StubCompleter {
        async fn complete_text(&self, _request: CompletionRequest) -> Result<String, HostError> {
            self.calls.fetch_add(1, Ordering::SeqCst);
            if let Some(ref name) = self.error_name {
                return Err(format!("stub completer {name} failing").into());
            }
            Ok(self.response.clone().unwrap_or_default())
        }
    }

    /// Minimal `WorkspaceWriter` stub that always fails — used to exercise the
    /// "archival failure preserves turns" path without needing a real backend.
    struct FailingWriter;

    #[async_trait]
    impl WorkspaceWriter for FailingWriter {
        async fn append(&self, _path: &str, _content: &str) -> Result<(), HostError> {
            Err("workspace append failed".into())
        }
    }

    fn make_compactor(llm: Arc<StubCompleter>) -> ContextCompactor {
    ...
    async fn test_compact_truncate_keeps_last_n() {
        let llm = Arc::new(StubCompleter::new("unused"));
    ...
    // ------------------------------------------------------------------
    // 2. compact_truncate with fewer turns than limit is a no-op
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 4. compact_with_summary produces summary turn via StubCompleter
    // ------------------------------------------------------------------

    #[tokio::test]
    async fn test_compact_with_summary_produces_summary_turn() {
        let canned_summary = "- User greeted the agent\n- Five exchanges completed";
        let llm = Arc::new(StubCompleter::new(canned_summary));
    ...
    // ------------------------------------------------------------------
    // 5. compact_with_summary: LLM failure propagates, thread unmodified
    // ------------------------------------------------------------------

    #[tokio::test]
    async fn test_compact_with_summary_llm_failure() {
        let llm = Arc::new(StubCompleter::failing("broken-llm"));
        let compactor = make_compactor(llm);
    ...
    async fn test_compact_with_summary_fewer_turns_than_keep() {
        let llm = Arc::new(StubCompleter::new("should not be called"));
        let compactor = make_compactor(llm.clone());
    ...
    // ------------------------------------------------------------------
    // 7. compact_with_summary preserves turns when workspace write fails
    //    (archival-failure fail-safe semantics)
    // ------------------------------------------------------------------

    #[tokio::test]
    async fn test_compact_with_summary_preserves_turns_when_workspace_write_fails() {
        let llm = Arc::new(StubCompleter::new("summary"));
        let compactor = make_compactor(llm.clone());
        let mut thread = make_thread(8);
        let ws = FailingWriter;
    ...
    // ------------------------------------------------------------------
    // 8. compact_to_workspace without workspace falls back to truncation
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 9. compact_to_workspace: fewer turns than keep is a no-op
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 10. compact_to_workspace preserves turns when workspace write fails
    // ------------------------------------------------------------------
    ...
    async fn test_compact_to_workspace_preserves_turns_when_workspace_write_fails() {
        ...
        let result = compactor
            .compact(&mut thread, CompactionStrategy::MoveToWorkspace, Some(&ws))
            .await
            .expect("compact should succeed even when workspace write fails");
    ...
    // ------------------------------------------------------------------
    // 11. format_turns_for_storage basic case
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 12. format_turns_for_storage includes tool calls
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 13. format_turns_for_storage with no response (incomplete turn)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 14. format_turns_for_storage empty list
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 15. Token counts decrease after truncation
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 16. compact_truncate with keep_recent=0 removes all turns
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 17. Summarize with keep_recent=0 summarizes all and removes all
    // ------------------------------------------------------------------

    #[tokio::test]
    async fn test_compact_with_summary_keep_zero() {
        let llm = Arc::new(StubCompleter::new("Summary of all turns"));
        let compactor = make_compactor(llm.clone());
        let mut thread = make_thread(5);
    ...
    // ------------------------------------------------------------------
    // 18. thread.messages() is coherent after compaction
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 19. Multiple sequential compactions work correctly
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // 20. CompactionPartial::empty() is a true zero state
    // ------------------------------------------------------------------
