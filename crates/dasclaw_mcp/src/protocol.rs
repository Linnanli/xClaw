//! MCP wire protocol types (JSON-RPC framing + capability negotiation).
//!
//! Ported from `desktop-client/ironclaw/src/tools/mcp/protocol.rs` (F3.2 / ADR-152).
//! Behaviour is preserved bit-for-bit; the only API change is that
//! [`McpRequest::initialize`] now takes the client identity as parameters so
//! that the wire `clientInfo.name` / `version` reflect the **host** crate, not
//! whatever crate happens to hold these types.
//!
//! The host should call [`McpRequest::initialize_with_client_info`] and pass
//! its own `CARGO_PKG_NAME` / `CARGO_PKG_VERSION`.
...
/// Default `clientInfo.name` used by [`McpRequest::initialize`].
///
/// The bare `initialize` helper is intended for tests and internal use; real
/// hosts (e.g. `ironclaw`) should call [`McpRequest::initialize_with_client_info`]
/// with their own package name + version so the wire identity is accurate.
pub const DEFAULT_CLIENT_NAME: &str = env!("CARGO_PKG_NAME");

/// Default `clientInfo.version` used by [`McpRequest::initialize`].
...
impl McpRequest {
    ...
    /// Create an initialize request with explicit client identity.
    ///
    /// `client_name` / `client_version` populate the `clientInfo` block sent
    /// to the MCP server; pass the **host** crate's `CARGO_PKG_NAME` /
    /// `CARGO_PKG_VERSION` (not this crate's) so the server sees an honest
    /// identity.
    pub fn initialize_with_client_info(id: u64, client_name: &str, client_version: &str) -> Self {
        Self::new(
            id,
            "initialize",
    ...
    /// Create an initialize request using this crate's identity.
    ///
    /// **For tests and internal use only.** Production hosts should call
    /// [`Self::initialize_with_client_info`] so the wire `clientInfo` reflects
    /// the host binary instead of `dasclaw_mcp`.
    pub fn initialize(id: u64) -> Self {
        Self::initialize_with_client_info(id, DEFAULT_CLIENT_NAME, DEFAULT_CLIENT_VERSION)
    }
...
mod tests {
    ...
    fn test_mcp_tool_missing_schema_gets_default() {
        let json = serde_json::json!({"name": "ping", "description": "Ping"});
        let tool: McpTool = serde_json::from_value(json).expect("deserialize McpTool");
    ...
    #[test]
    fn test_initialize_request_with_client_info_uses_host_identity() {
        let req = McpRequest::initialize_with_client_info(42, "ironclaw", "0.24.0");
        let params = req.params.expect("initialize must have params");
        assert_eq!(params["clientInfo"]["name"], "ironclaw");
        assert_eq!(params["clientInfo"]["version"], "0.24.0");
    ...
    #[test]
    fn test_initialize_default_uses_dasclaw_mcp_identity() {
        let req = McpRequest::initialize(1);
        let params = req.params.expect("initialize must have params");
        assert_eq!(params["clientInfo"]["name"], DEFAULT_CLIENT_NAME);
        assert_eq!(params["clientInfo"]["version"], DEFAULT_CLIENT_VERSION);
    }
    ...
    fn test_response_with_string_id() {
        let json = serde_json::json!({"jsonrpc": "2.0", "id": "42", "result": {}});
    ...
    fn test_response_with_non_numeric_string_id() {
        let json = serde_json::json!({"jsonrpc": "2.0", "id": "not-a-number", "result": {}});
        let resp: McpResponse = serde_json::from_value(json).expect("deserialize");
        assert_eq!(resp.id, None);
    }
    ...
    fn test_execution_time_hint_serde() {
        for (s, v) in [
            ("fast", ExecutionTimeHint::Fast),
            ("medium", ExecutionTimeHint::Medium),
            ("slow", ExecutionTimeHint::Slow),
        ] {
            let hint: ExecutionTimeHint =
                serde_json::from_value(serde_json::json!(s)).expect("deserialize");
            assert_eq!(hint, v);
            assert_eq!(serde_json::to_value(hint).expect("serialize"), s);
        }
    }
}
