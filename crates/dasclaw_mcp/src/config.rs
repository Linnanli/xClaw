//! MCP server configuration (host-agnostic types + path-based persistence).
//!
//! Holds only the portable surface: the `McpServerConfig` / `OAuthConfig` /
//! `McpServersFile` data types, validation, and explicit-path JSON
//! load/save. The default-path wrappers and any DB-backed wrappers live
//! in the host crate that owns the filesystem layout and database schema.
//!
//! Migrated from `desktop-client/ironclaw/src/tools/mcp/config.rs`
//! in F3.2 phase 2 PR 5a (#641).
...
use crate::server_name::McpServerName;
