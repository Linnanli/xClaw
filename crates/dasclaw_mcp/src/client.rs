//! MCP client (host-agnostic) for connecting to MCP servers.
//!
//! Supports both local (unauthenticated) and hosted (OAuth-authenticated)
//! servers. Uses pluggable transports (HTTP, stdio, Unix) via the
//! `McpTransport` trait.
//!
//! Migrated from `desktop-client/ironclaw/src/tools/mcp/client.rs` in
//! F3.2 phase 3 PR 6-γ (#661, #666). The `impl Tool for McpToolWrapper`
//! adapter layer (which depends on the ironclaw-specific `Tool` trait,
//! `JobContext`, `ApprovalRequirement`, and `ToolOutput`) and the
//! `create_tools()` convenience method stay in ironclaw at
//! `tools/mcp/client_tool.rs`.
//!
//! OAuth-proxy refresh is injected via [`RefreshAccessTokenFn`]; when not
//! provided, the client falls back to
//! [`crate::auth::refresh_access_token_direct`] (the protocol-layer
//! refresh against the authorization server's token endpoint).
...
/// Host-supplied OAuth access-token refresh hook.
///
/// When the client detects an expired token (either at read time or after
/// a 401 from an HTTP transport), it invokes this closure to obtain a fresh
/// access token. If no hook is set, the client falls back to
/// [`crate::auth::refresh_access_token_direct`], which performs a direct
/// RFC 6749 refresh against the authorization server's token endpoint.
///
/// The ironclaw host installs a hook that additionally consults the
/// optional OAuth-proxy refresh path; see
/// `ironclaw::tools::mcp::client_tool::ironclaw_refresh_fn` (PR 6-γ, #666).
pub type RefreshAccessTokenFn = std::sync::Arc<
    dyn Fn(
            crate::config::McpServerConfig,
            std::sync::Arc<dyn dasclaw_runtime::secrets::SecretsStore + Send + Sync>,
            String,
        ) -> futures::future::BoxFuture<
            'static,
            Result<crate::auth::AccessToken, crate::auth::AuthError>,
        > + Send
        + Sync,
>;
...
pub struct McpClient {
    ...
    /// Host-supplied OAuth refresh hook. Falls back to
    /// `dasclaw_mcp::auth::refresh_access_token_direct` when `None`.
    refresh_fn: Option<RefreshAccessTokenFn>,
}
...
impl McpClient {
    ...
    pub fn new_with_config(config: McpServerConfig) -> Result<Self, ToolError> {
        if !matches!(
            ...
            crate::config::EffectiveTransport::Http
        ) {
    ...
    /// Install a host-supplied OAuth refresh hook.
    ///
    /// See [`RefreshAccessTokenFn`] for semantics. Returns `self` to chain
    /// with other builder methods.
    pub fn with_refresh_fn(mut self, refresh_fn: RefreshAccessTokenFn) -> Self {
        self.refresh_fn = Some(refresh_fn);
        self
    }
    ...
    /// Invoke the host-supplied OAuth refresh hook, falling back to the
    /// direct (RFC 6749) refresh when no hook is installed.
    async fn refresh_access_token_via_hook(
        &self,
        config: &McpServerConfig,
        secrets: &Arc<dyn SecretsStore + Send + Sync>,
    ) -> Result<crate::auth::AccessToken, crate::auth::AuthError> {
        if let Some(ref f) = self.refresh_fn {
            f(config.clone(), Arc::clone(secrets), self.user_id.clone()).await
        } else {
            crate::auth::refresh_access_token_direct(config, secrets, &self.user_id).await
        }
    }
    ...
    async fn get_access_token(&self) -> Result<Option<String>, ToolError> {
            ...
            .await
        {
            ...
            Err(dasclaw_runtime::secrets::SecretError::NotFound(_)) => Ok(None),
            Err(dasclaw_runtime::secrets::SecretError::Expired) => {
                // Token expired — attempt refresh before failing.
                tracing::info!(
                    server = %self.server_name,
                    "Access token expired, attempting refresh"
                );
                match self.refresh_access_token_via_hook(config, secrets).await {
                    Ok(new_token) => {
                        tracing::info!(
                            server = %self.server_name,
...
mod tests {
    ...
    async fn test_build_headers_skips_empty_token() {
        ...
        #[async_trait]
        impl dasclaw_runtime::secrets::SecretsStore for EmptyTokenStore {
            async fn create(
                &self,
                _user_id: &str,
    ...
    async fn test_build_headers_trims_token() {
        ...
        #[async_trait]
        impl dasclaw_runtime::secrets::SecretsStore for PaddedTokenStore {
            async fn create(
                &self,
                _user_id: &str,
                _params: CreateSecretParams,
