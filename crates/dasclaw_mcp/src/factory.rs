...
//!
//! Migrated from `desktop-client/ironclaw/src/tools/mcp/factory.rs` in
//! F3.2 phase 3 PR 6-γ (#661, #666). The ironclaw shim at
//! `tools/mcp/factory.rs` installs the host-side `RefreshAccessTokenFn`
//! (OAuth-proxy aware) before returning the client.

use std::sync::Arc;

use crate::config::{EffectiveTransport, McpServerConfig};
use crate::http_transport::HttpMcpTransport;
use crate::{McpClient, McpProcessManager, McpSessionManager, McpTransport};
...
pub async fn create_client_from_config(
    ...
    match server.effective_transport() {
        ...
        EffectiveTransport::Unix { socket_path } => {
            let transport =
                crate::unix_transport::UnixMcpTransport::connect(&server_name, socket_path)
                    .await
        ...
        EffectiveTransport::Http => {
            ...
            if let Some(ref secrets) = secrets {
                let has_tokens = crate::auth::is_authenticated(&server, secrets, user_id).await;
