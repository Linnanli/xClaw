...
impl RecordingLlm {
    ...
    ///
    /// In the shared crate, this is a no-op stub. The full implementation
    /// lives in ironclaw's agent layer which has access to Workspace.
    pub async fn snapshot_memory(&self, _workspace: &dyn std::any::Any) {
        tracing::debug!("snapshot_memory: no-op in ironclaw_llm shared crate");
    }
