...
use crate::oauth_helpers::OAUTH_CALLBACK_PORT;
...
pub struct SessionManager {
    ...
    store: RwLock<Option<Arc<dyn crate::util::SettingsStore>>>,
...
impl SessionManager {
    ...
    pub async fn attach_store(&self, store: Arc<dyn crate::util::SettingsStore>, user_id: &str) {
    ...
    async fn validate_token(&self) -> Result<(), LlmError> {
        ...
        let preview = crate::util::truncate_for_preview(&body, 200);
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // 共享 crate 中不支持交互式输入（无法依赖 ironclaw 主 crate 的 setup 模块）。
        // ironclaw 主 crate 通过覆盖此方法提供完整的交互式 API key 输入流程。
        Err(LlmError::SessionRenewalFailed {
            provider: "nearai".to_string(),
            reason: "Interactive API key input not supported in shared crate".to_string(),
        })
    }
