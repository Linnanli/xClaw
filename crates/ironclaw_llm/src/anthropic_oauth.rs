...
use crate::retry::cap_retry_after;
    ...
    async fn send_request<R: for<'de> Deserialize<'de>>(
        ...
        if !status.is_success() {
            ...
            if status.as_u16() == 401 {
                ...
                if let Some(fresh) = Option::<secrecy::SecretString>::None {
                    ...
                    if retry.status().is_success() {
                        ...
                        return serde_json::from_str(&text).map_err(|e| {
                            let truncated = crate::util::truncate_for_preview(&text, 512);
            ...
            let truncated = crate::util::truncate_for_preview(&response_text, 512);
    ...
    fn test_retry_after_large_number() {
        ...
        assert_eq!(
            ...
            Some(std::time::Duration::from_secs(
                crate::retry::MAX_RETRY_AFTER_SECS
            ))
        );
    }
