//! Line-delimited stdio JSON-RPC entrypoint for the Phase 1 app-server skeleton.
//!
...
fn self_check_report() -> Result<SelfCheckReport, dasclaw_app_server::AppServerError> {
    ...
    let turn_start = server.turn_start(TurnStartParams {
        thread_id: thread_create.thread_id.clone(),
        prompt: "self-check prompt is not persisted by the skeleton".to_string(),
    })?;
...
fn run_stdio_server<R, W>(reader: R, mut writer: W) -> io::Result<()>
where
    R: BufRead,
    W: Write,
{
    ...
    for line in reader.lines() {
        ...
        if let Some(response) = server.handle_json_rpc(&line) {
...
mod tests {
    ...
    #[test]
    fn stdio_loop_writes_notifications_after_response() {
        ...
        let first: Value = serde_json::from_str(lines[0]).expect("response should be JSON");
        let initializing: Value =
            serde_json::from_str(lines[1]).expect("initializing notification should be JSON");
        let ready: Value =
            serde_json::from_str(lines[2]).expect("ready notification should be JSON");
        let capabilities: Value =
            serde_json::from_str(lines[3]).expect("capability notification should be JSON");

        assert_eq!(first["result"]["lifecycle"]["state"], "ready");
    ...
    fn stdio_loop_keeps_server_state_across_lines() {
        ...
        let init_response: Value = serde_json::from_str(lines[0]).expect("init response JSON");
    ...
    fn stdio_loop_exits_after_shutdown() {
        ...
        let shutdown_response: Value =
            serde_json::from_str(lines[0]).expect("shutdown response should be JSON");
        let lifecycle_notification: Value =
            serde_json::from_str(lines[1]).expect("shutdown notification should be JSON");
