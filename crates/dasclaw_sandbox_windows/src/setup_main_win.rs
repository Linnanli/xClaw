...
//
// This file is the source body of the `dasclaw-sandbox-setup` bin entry
// (`src/bin/setup_main.rs`) per ADR-130 §2. It is included via
// `#[path = "../setup_main_win.rs"] mod win;` and is NOT a lib module.
// The `mod firewall;` / `mod read_acl_mutex;` / `mod sandbox_users;`
// declarations resolve, via the `#[path]` import, to sibling files in
// `crates/dasclaw_sandbox_windows/src/` -- the same source files compiled
// into the lib in earlier waves; they are intentionally double-compiled
// (once into the lib, once into this bin) to mirror upstream byte-for-byte.
...
use dasclaw_sandbox_windows::LOG_FILE_NAME;
use dasclaw_sandbox_windows::SETUP_VERSION;
use dasclaw_sandbox_windows::SetupErrorCode;
use dasclaw_sandbox_windows::SetupErrorReport;
use dasclaw_sandbox_windows::SetupFailure;
use dasclaw_sandbox_windows::add_deny_write_ace;
use dasclaw_sandbox_windows::canonicalize_path;
use dasclaw_sandbox_windows::convert_string_sid_to_sid;
use dasclaw_sandbox_windows::ensure_allow_mask_aces_with_inheritance;
use dasclaw_sandbox_windows::ensure_allow_write_aces;
use dasclaw_sandbox_windows::extract_setup_failure;
use dasclaw_sandbox_windows::hide_newly_created_users;
use dasclaw_sandbox_windows::is_command_cwd_root;
use dasclaw_sandbox_windows::load_or_create_cap_sids;
use dasclaw_sandbox_windows::log_note;
use dasclaw_sandbox_windows::path_mask_allows;
use dasclaw_sandbox_windows::sandbox_bin_dir;
...
use dasclaw_sandbox_windows::workspace_cap_sid_for_cwd;
use dasclaw_sandbox_windows::write_setup_error_report;
