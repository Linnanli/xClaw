...
- **2026-06-XX (amendment 3)**: issue [#874](https://github.com/Linnanli/xClaw/issues/874) 实施期间，发现 `permissions.rs` 的 kernel-layer read-only carve-out 列举 `.git` / `.agents` / `.codex` 三个 codex-native meta 目录，缺失 dasclaw-native 的 `.dasclaw`。决策层 (`dasclaw_workspace_cap::policy::default_read_only_subpaths_for_writable_root`) 已有 `.dasclaw`，kernel 层不对等会导致 dasclaw 元数据无写保护。修复需要在 verbatim-locked 文件里注入 4 行 dasclaw-only 逻辑，原 §3 + ADR-129 §1.3 仅允许 (i) Cargo.toml 名称交换 / (ii) `codex_*` use-path 重写。本 amendment 3 在两条机械适配之外新增第 (iii) 条**「marker-wrapped 本地扩展」**：以 `// CODEX-DRIFT-IGNORE-START: <reason>` ... `// CODEX-DRIFT-IGNORE-END` 成对注释包裹的代码块由 `scripts/check_codex_protocol_drift.py` 在哈希比对前剥离，START 注释必须 cite 一个 issue 或 ADR 以供 PR review 审计为有意的本地扩展，而非 upstream drift。约束：(a) 不允许在内联 `#[cfg(test)] mod tests` 之外的位置开任何**与 codex 现有行为相冲突**的代码块；本机制只供"增量并列"语义（如 `.codex` 之后追加 `.dasclaw` 兄弟分支）；(b) 每个 marker 块必须 cite 来源 issue/ADR；(c) drift 脚本对未配对的 START/END 一律 fail-loudly；(d) inline test 中若新增的运行时分支会改变现有断言，可同样用 marker 包裹补齐断言，保持 drift 透明、运行时一致。落地：`permissions.rs` 在 `default_read_only_subpaths_for_writable_root` 与 `FileSystemSandboxPolicy::from` 各加一个 marker 块；`protocol.rs` 在 3 个相关 inline test 中补齐 `.dasclaw` 期望；新增 `crates/dasclaw_protocol/tests/dasclaw_meta_carve_out.rs` 4 个集成测试落地"`.dasclaw` 在 read-only 集中"行为；ADR-129 §1.3.1（"local mechanical adaptations"）授权依据。next: 本 PR merge 后将该 marker 机制视为 file-level slice crate 的标准本地扩展通道，未来类似 `.dasclaw` 类语义并列的场景沿用同一机制。
