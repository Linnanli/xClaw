...
| P3 | Approval + tool + sandbox 三件套 | ~~ServerRequest request tracking、approval decision、tool lifecycle notification、sandbox capability gate、fail-safe timeout~~；~~P5a standalone `fs/*` / buffered `command/exec` 已接~~；dynamic `item/tool/call`、file-change approval、command PTY follow-up 仍留给后续专项 | Codex 大量协议依赖这组能力；安全控制闭环先于 command/fs owner，后续继续补 tool/file/PTY 边界 |
