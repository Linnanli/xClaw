...
|---|---|---|---|---|
| R1 | Runtime tool / approval owner | ~~`item/tool/call`~~、~~`item/tool/requestUserInput`~~、~~`item/permissions/requestApproval`~~、~~`item/fileChange/requestApproval`~~、~~`item/autoApprovalReview/started`~~、~~`item/autoApprovalReview/completed`~~、~~file-change output / patch events~~；后续仍需把真实 file-change / user-input / auto-review producer 与客户端 UI / 客户端工具执行器补到端到端产品可用闭环 | 已由 `docs/superpowers/plans/2026-06-22-dasclaw-app-server-runtime-tool-approval-owner.md` 接入 protocol types、runtime producer、pending server request routing、desktop fail-closed 转发和 owner-ready capability gating；当前删除线表示 app-server 控制面 / 安全回填链路完成，不表示 renderer 已具备 user-input 表单、patch 审批 UI 或客户端 tool executor | `capabilities/list` 只在 runtime bridge 报告 R1 owner-ready 时广告；runtime producer 能发对应 ServerRequest / notification；JSON-RPC response 能回填执行路径；拒绝、超时、malformed response、shutdown 全部 fail-safe 且有测试；后续完成判定应补 renderer 交互测试和客户端工具执行器产品闭环，且不能用自动 reject / 空 answers 冒充用户可用能力 |
