...
| `crates/dasclaw_app_server_protocol/src/lib.rs` | `codex_app_server_v2_chat_session_subset` profile 明确是 `ChatSessionSubset`，列出当前真实 route 的 chat-session、thread lifecycle、thread goal 与 model 方法；仍 opt out 完整 tool / approval / sandbox / compact 等未完整实现的 Codex 能力 |
...
| Thread / turn | ~~`thread/started`~~、~~`thread/status/changed`~~、~~`thread/archived`~~、~~`thread/unarchived`~~、~~`thread/name/updated`~~、~~`thread/goal/updated`~~、~~`thread/goal/cleared`~~、~~`thread/tokenUsage/updated`~~、~~`turn/started`~~、~~`turn/completed`~~ |
...
| R6 | Config / repo tools / search owner | ~~broad `config/read`、`config/value/write`、`config/batchWrite`~~；~~`gitDiffToRemote`~~；~~`fuzzyFileSearch` session~~；~~`getConversationSummary`~~；~~`review/start`~~；model reroute producer / model verification producer；~~HookRegistry `preToolUse` hook events~~ / remaining hook lifecycle events；~~config/deprecation warning producer~~ / `warning` startup producer / guardian warning producer | R6 app-server owner 已完成 config allowlist policy、read-only repo diff service、path fuzzy search、deterministic conversation summary、review-start routing、`beforeToolCall` -> `preToolUse` HookRegistry observer events，以及 config/deprecation warning producers（启动期和运行期 config RPC 都能送达）。`model/rerouted` 目前只有 runtime delivery，provider `actual_model` 不再被误标成 `highRiskCyberActivity`；`model/verification` 仍缺真实 provider metadata source；通用 startup `warning` 缺真实 sandbox policy source；`guardianWarning` 缺真实 guardian/safety runtime source；其它 HookRegistry lifecycle 也尚未接入真实 turn/session 路径。Dasclaw 既有 git/search/hooks/core/config 底座被复用；Codex product-only 的 external-agent / feedback / experiment 等域仍留在 R7，不混入 R6。 | 已完成子项有 method/schema、service owner、fail-closed 错误语义和非 fake-only producer 测试；`model/rerouted`、`model/verification`、通用 `warning`、`guardianWarning` 与剩余 hook lifecycle 从剩余项移除前必须接真实上游 source、capability readiness 和非 fake-only 测试 |
