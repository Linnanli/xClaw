...
| R6 | Config / repo tools / search owner | ~~broad `config/read`、`config/value/write`、`config/batchWrite`~~；~~`gitDiffToRemote`~~；~~`fuzzyFileSearch` session~~；~~`getConversationSummary`~~；~~`review/start`~~；model reroute / verification producer；hook/warning producer | R6 app-server owner 已完成 config allowlist policy、read-only repo diff service、path fuzzy search、deterministic conversation summary 与 review-start routing。Typed protocol 和 notification drain 已存在，但 model reroute / verification 目前只有 runtime update sink 转发，没有真实 app-server producer / readiness；hook/warning 目前只有队列 drain，未接 `dasclaw_hooks::HookRegistry` 或 warning source，故不广告为完成能力。Dasclaw 既有 git/search/hooks/core/model 底座被复用；Codex product-only 的 external-agent / feedback / experiment 等域仍留在 R7，不混入 R6。 | 已完成子项有 method/schema、service owner、fail-closed 错误语义和测试；model/hook/warning 从剩余项移除前必须接真实 producer、capability readiness 和非 fake-only 测试 |
