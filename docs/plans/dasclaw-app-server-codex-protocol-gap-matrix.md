...
| R6 | Config / repo tools / search owner | broad `config/read`、`config/value/write`、`config/batchWrite`；`gitDiffToRemote`；`fuzzyFileSearch` session；`getConversationSummary`；`review/start`；model reroute / verification；hooks warning channel | R6 app-server owner 已完成：typed protocol、config allowlist policy、read-only repo diff service、path fuzzy search、deterministic conversation summary、review-start routing、model event delivery、hook/warning notification drain 均已接线。Dasclaw 既有 git/search/hooks/core/model 底座被复用；Codex product-only 的 external-agent / feedback / experiment 等域仍留在 R7，不混入 R6。 | Method/event schema 列出全部 R6 contract；service tests 覆盖成功路径和 fail-closed 错误语义；R7 产品域继续 explicit unsupported / opt-out |
