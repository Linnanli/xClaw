...
| Filesystem | ~~`fs/readFile`~~、~~`fs/writeFile`~~、~~`fs/createDirectory`~~、~~`fs/getMetadata`~~、~~`fs/readDirectory`~~、~~`fs/remove`~~、~~`fs/copy`~~、~~`fs/watch`~~、~~`fs/unwatch`~~ | P5a 已由 `AppServerFsService` 接管，并通过 real JSON-RPC route tests 覆盖 read/write/list/stat/copy/remove、containment 和 `fs/changed` drain | A/D2 | 基础 app-server filesystem owner 已完成；fuzzy search、file-change approval UI、审计/权限产品语义仍是后续专项 |
| Command exec | ~~`command/exec`~~、`command/exec/write`、`command/exec/terminate`、`command/exec/resize` | P5a 已由 `AppServerCommandExecService` 接管 buffered `command/exec`，经 `SandboxedShellExecutor` 执行并发出 `command/exec/outputDelta`；write/terminate/resize 在无 sandboxed PTY/long-running process owner 前保持 fail-safe unavailable | A/D1 | 已完成 buffered command exec control-plane；PTY/进程生命周期、stdin/resize/terminate streaming 仍需后续 service |
...
| P5 | Filesystem / command exec | ~~`fs/*`~~、~~buffered `command/exec`~~、~~`fs/changed`~~、~~`command/exec/outputDelta`~~；`command/exec/write`、`command/exec/terminate`、`command/exec/resize` 仍需 sandboxed PTY / long-running process owner | P5a 已在 P3 安全边界之后接入 app-server-owned filesystem 与 buffered command exec；streaming/PTY follow-up 不冒充 implemented capability |
