...
pub async fn quota_check(
    ...
    let Some(did) = dept_id else {
        return Ok(Json(QuotaCheckResponse {
            ...
            root_daily_used_cents: None,
            root_daily_limit_cents: None,
        }));
    };
    ...
    let (allowed, reason) = if !dept_ok {
        (
            false,
            Some("当日费用已达部门限额，请联系管理员".to_string()),
        )
    } else if !root_ok {
        (
            false,
            Some("当日费用已达公司级限额，请联系管理员".to_string()),
        )
    } else {
        (true, None)
    };
...
pub async fn quota_overview(State(state): State<AppState>) -> Result<Json<serde_json::Value>> {
    let client = state
        .db_pool
        .get()
        .await
    ...
    let budget_usage_pct = monthly_budget.map(|b| {
        if b > 0 {
            (month_cost as f64 / b as f64 * 100.0) as i32
        } else {
            0
        }
    });

    let active_models: i64 = client
        .query_one(
...
pub async fn get_quota_config(State(state): State<AppState>) -> Result<Json<serde_json::Value>> {
    let client = state
        .db_pool
        .get()
        .await
...
pub async fn department_ranking(State(state): State<AppState>) -> Result<Json<serde_json::Value>> {
    let client = state
        .db_pool
        .get()
        .await
...
pub async fn model_ranking(State(state): State<AppState>) -> Result<Json<serde_json::Value>> {
    let client = state
        .db_pool
        .get()
        .await
...
async fn query_user_department(
    ...
    let row = client
        .query_opt("SELECT department_id FROM users WHERE id = $1", &[&user_id])
        .await
        .map_err(|e| Error::Database(e.to_string()))?;
...
/// 最多遍历 20 层防止循环引用
async fn find_root_department(client: &deadpool_postgres::Object, dept_id: Uuid) -> Result<Uuid> {
...
async fn check_quota_warning(
    ...
    let usage = query_dept_daily_usage(&client, dept_id, &today)
        .await
