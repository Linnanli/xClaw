...
pub async fn get_compliance_overview(
    ...
    // 总 DLP 规则数（用于计算覆盖率）
    let total_rules: i64 = client
        .query_one("SELECT COUNT(*) FROM dlp_rules WHERE enabled = true", &[])
        .await
        .map_err(|e| Error::Database(e.to_string()))?
        .get(0);
...
async fn build_report_content(
    ...
    // 审批工单数
    let approval_tickets: i64 = client
        .query_one(
