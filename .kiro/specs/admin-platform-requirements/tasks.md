...
- [ ] 19.1 执行 `020_watermark.sql`：创建 watermark_config 表，插入默认配置
- [ ] 19.2 实现水印配置 API：`GET/PUT /api/watermark/config`（需求 20.1）
- [ ] 19.3 在系统设置页添加水印配置 Tab：启用开关、内容模板、字体大小、透明度、位置（需求 20.1）
- [ ] 19.4 [Desktop Client] 前端实现水印渲染：导出对话时将模板变量替换为用户信息并渲染可见水印（需求 20.5）

### 20. 统计报表扩展（需求 12 新增部分）

- [ ] 20.1 实现 AI 使用量报表 API：`GET /api/reports/ai-usage`，支持日/周/月维度（需求 12.5）
- [ ] 20.2 实现模型费用统计报表 API（需求 12.6）
- [ ] 20.3 实现部门 AI 使用量排行 API（需求 12.7）
- [ ] 20.4 前端报表页：添加 AI 使用量趋势图、模型费用统计、部门排行（需求 12.5-12.7）
- [ ] 20.5* 实现报表导出为 PDF 或 Excel 格式（需求 12.8）
